...
| drop drop this int he same folder as Angabe 6
| and execute
...
import           Angabe6           (Anbieter (..), Datensatz (..), EUR (..),
                                    Haendler (..), Jahr (..),
                                    Lieferausblick (..), Lieferfenster (..),
                                    Quartal (..), Skonto (..), Sortiment (..),
                                    Typ (..), Waescheschleuder (..),
                                    Waeschetrockner (..), Waschmaschine (..),
                                    Wgf (..), guenstigste_Lieferanten,
...
spec =
    ...
    "[ A.1 - 1 ]"  ~: length (wg_la lab1)  @?= 12,
    "[ A.1 - 2 ]"  ~: length (wg_so sort1)  @?= 12,
    "[ A.1 - 3 ]"  ~: length (wg_ab anb1)  @?= 10,

    "[ A.2 - 1 ]"  ~: ist_wgf  lab6  @?= True,
    "[ A.2 - 2 ]"  ~: ist_nwgf lab3  @?= False,
    "[ A.2 - 3 ]"  ~: ist_wgf  lab0  @?= True,
    "[ A.2 - 4 ]"  ~: ist_wgf  sort1 @?= True,
    "[ A.2 - 5 ]"  ~: ist_nwgf sort3 @?= False,
    "[ A.2 - 6 ]"  ~: ist_nwgf sort0 @?= False,
    "[ A.2 - 7 ]"  ~: ist_wgf  anb1  @?= True,
    "[ A.2 - 8 ]"  ~: ist_nwgf anb2  @?= False,
    "[ A.2 - 9 ]"  ~: ist_nwgf anb0  @?= False,

    -- checking for errors is really weird only works for me if I
    -- artificially slow down wg_la, wg_so, wg_ab
    -- might work for some

    -- "[ A.1 - 1 ]" ~:  assertError "Ausblickfehler"  (wg_la $ wgf_fehler lab1),
    -- "[ A.1 - 2 ]" ~:  assertError "Sortimentfehler" (fst . head . wg_so $ wgf_fehler sort1),
    -- "[ A.1 - 3 ]" ~:  assertError "Anbieterfehler"  (fst . head . wg_ab $ wgf_fehler anb1),
    ...
    "[ A.7 - 1 ]"  ~: guenstigste_Lieferanten (M M1) (LF Q1 J2023) anb1 @?= Just [H5, H2],
    "[ A.7 - 2 ]"  ~: guenstigste_Lieferanten (T T3) (LF Q2 J2023) anb1 @?= Just [H7],
    "[ A.7 - 3 ]"  ~: guenstigste_Lieferanten (T T4) (LF Q3 J2023) anb2 @?= Just [H3, H1],
    "[ A.7 - 4 ]"  ~: guenstigste_Lieferanten (S S3) (LF Q4 J2023) anb2 @?= Nothing,
