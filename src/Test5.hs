...
import           Control.Exception (ErrorCall (ErrorCallWithLocation), evaluate,
                                    try)
import           Test.HUnit        (Test (TestCase), Testable (test),
                                    assertFailure, runTestTTAndExit, (~:),
                                    (~?=))
...
sort1  = [(M M2,ds1),(M M3,ds1),(M M4,ds1),(M M5,ds1),(T T2,ds1),(T T3,ds1),(T T4,ds1),(S S2,ds1),(S S3,ds0)]
sort2  = [(M M1,ds2),(M M2,ds2),(M M3,ds2),(M M4,ds2),(M M5,ds2)]
sort3  = [(T T1,ds2),(T T2,ds2),(T T3,ds2),(T T4,ds2)]
sort4  = [(S S1,ds2),(S S2,ds2),(S S3,ds4)]
sort5  = [(M M1,ds2),(T T1,ds2),(S S1,ds2)]
sort6  = [(M M2,ds3),(T T2,ds3),(S S2,ds3)]
sort7  = [(M M3,ds3),(T T3,ds3)]
sort8  = [(M M4,ds3),(T T4,ds3)]
sort9  = [(M M5,ds3)]
sort10 = [(M M5,ds2),(T T4,ds2),(S S3,ds4)]
sortf1 = [(M M5,ds0),(M M1,ds0),(M M5,ds0)] --2* M M5
sortf2 = [(M M1,ds0),(M M2,ds0),(M M3,ds0),(M M4,ds0),(M M5,ds0),(T T1,ds0),(T T2,ds0),(T T3,ds0),(T T3,ds0),(T T4,ds0),(S S1,ds0),(S S2,ds0),(S S3,ds0)] --2* T t3
sortf3 = [(M M5,dsf)] -- faulty Lieferausblick
...
tests = [
        ...
        "[ A.6 - f1 ]"~: TestCase $ assertError "Anbieterfehler" (sofort_erhaeltliche_Stueckzahl (S S3) anbf1),
        "[ A.6 - f2 ]"~: TestCase $ assertError "Anbieterfehler" (sofort_erhaeltliche_Stueckzahl (M M2) anbf2),

        "[ A.7 - f1 ]"~: TestCase $ assertError "Anbieterfehler" (guenstigste_Lieferanten (M M5) (LF Q1 2023) anbf1),
        "[ A.7 - f1 ]"~: TestCase $ assertError "Anbieterfehler" (guenstigste_Lieferanten (M M5) (LF Q1 2023) anbf2),
        "[ A.7 - f1 ]"~: TestCase $ assertError "Ausblickfehler" (guenstigste_Lieferanten (M M5) (LF Q1 2023) anbf3),
        "[ A.7 - f2, not accessed, throwing this error means your code could be lazier]" ~: guenstigste_Lieferanten (M M1) (LF Q1 2023) anbf3 ~?= Just [H5,H2],

        "[ A.8 - f1 ]"~: TestCase $ assertError "Anbieterfehler" (guenstigste_Lieferanten_im_Lieferfenster (M M5) (LF Q1 2023) 0 anbf1),
        "[ A.8 - f1 ]"~: TestCase $ assertError "Anbieterfehler" (guenstigste_Lieferanten_im_Lieferfenster (M M5) (LF Q1 2023) 0 anbf2),
        "[ A.8 - f2 ]"~: TestCase $ assertError "Ausblickfehler" (guenstigste_Lieferanten_im_Lieferfenster (M M5) (LF Q1 2023) 1 anbf3),
        "[ A.8 - f3, not accessed, throwing this error means your code could be lazier]" ~: guenstigste_Lieferanten_im_Lieferfenster (M M1) (LF Q1 2023) 0 anbf3 ~?= [(H5,EUR {euro = 0}),(H2,EUR {euro = 0})],
